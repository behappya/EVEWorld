...
- **Detection stack location.** `mlr_eval.py` imports `t4g_detect`, `t4g_exam_v2`, and `t4g_gdino`; in this release these modules live in [`eveworld/pipeline/`](../../eveworld/pipeline/), so the import-root constant at the top of `mlr_eval.py` must resolve there.
