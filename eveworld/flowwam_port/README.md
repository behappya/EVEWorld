...
- **GDINO import path.** `gdino_annotate.py` inserts `../track4gen` on `sys.path` to import `t4g_gdino.GDinoLocator`; in this repo the module lives at [`eveworld/pipeline/t4g_gdino.py`](../pipeline/t4g_gdino.py) — symlink or adjust `PYTHONPATH` accordingly.
...
- **Dispatch helper.** Some cluster wrappers reference an `arm_generate_dispatch.py` fan-out helper that is not shipped in this directory; use `arm_generate.py --shard i/n` directly (it is idempotent per output video).
